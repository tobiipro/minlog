...
class MinLog {
  ...
  child(childOptions = {}) {
    let serializers = _lodashFirecloud.default.concat([], this.serializers, childOptions.serializers);
    let listeners = _lodashFirecloud.default.concat([], this.listeners, childOptions.listeners);
  ...
  async log(levelCodeOrName, ...args) {
    ...
    if (this.requireSrc) {
      ...
      let externalCallSite = _lodashFirecloud.default.find(callSites, function (callSite) {
        return callSite.getFileName() !== __filename;
      });
      if (_lodashFirecloud.default.isDefined(externalCallSite)) {
        src = {
