...
exports.default = function (cfg = {}) {
  return (() => {
    var _ref = (0, _bluebird.coroutine)(function* ({ entry, logger, rawEntry }) {
      if (_lodash2.default.filter(rawEntry._args).length === 1 && rawEntry._args[0]._babelSrc) {
        return;
      }

      let maxLevelName = cfg.level || 'trace';
      let maxLevel = logger.levels[maxLevelName];
      maxLevel = _lodash2.default.floor(maxLevel / 10) * 10 + 10 - 1; // round up to next level, not inclusive
      ...
      let formattedLevelName = _lodash2.default.padStart(_lodash2.default.toUpper(levelName), '5');
      ...
      let src = '';
      if (entry._babelSrc) {
        src = _lodash2.default.merge({}, entry._src, entry._babelSrc);
      ...
      let iframeId = _lodash2.default.defaultTo(cfg.iframeId, window.parent === window ? 'top' : '?');
      ...
      let extra = _lodash2.default.omit(rawEntry, ['_args', '_babelSrc', '_level', '_src', '_time', 'iframeId', 'msg']);
      _lodash2.default.merge(extra, context);

      // devTools console sorts keys when object is expanded
      extra = _lodash2.default.toPairs(extra);
      extra = _lodash2.default.sortBy(extra, 0);
      extra = _lodash2.default.fromPairs(extra);

      // devTools collapses objects with 'too many' keys,
      // so we output objects with only one key
      _lodash2.default.forEach(extra, function (value, key) {
        extraArgs.push('\n');
...
let _levelToConsoleFun = exports._levelToConsoleFun = function ({ level, levels }) {
  if (_lodash2.default.isString(level)) {
    // eslint-disable-next-line prefer-destructuring
    level = levels[level];
  }

  if (_lodash2.default.inRange(level, 0, levels.warn)) {
    return 'error';
  } else if (_lodash2.default.inRange(level, levels.warn, levels.info)) {
    return 'warn';
  } else if (_lodash2.default.inRange(level, levels.info, levels.debug)) {
    return 'info';
  } else if (_lodash2.default.inRange(level, levels.debug, levels.trace)) {
    // return 'debug';
